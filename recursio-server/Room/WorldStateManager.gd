...
func _physics_process(_delta):
	if _character_manager.players.size() >= 2:
...
func _create_world_state():
	...
	var player_states = {}
	for player_id in _character_manager.players:
		...
		player_state.position = _character_manager.players[player_id].transform.origin
		player_state.velocity = _character_manager.players[player_id].velocity
		player_state.acceleration = _character_manager.players[player_id].acceleration
		player_state.rotation = _character_manager.players[player_id].rotation.y
		player_state.rotation_velocity = _character_manager.players[player_id].rotation_velocity
